...
class Nav_bar_search extends React.Component {
  ...
  componentDidMount(){
    ...
    const tt = url_array[1].split("&")
    console.log( tt[0], url_array[2])
    const term = tt[0];
    const location = url_array[2];
    ...
    if (Boolean(this.state.fetchedData)) {
      ...
      if (data.businesses !== undefined) {
        ...
        data.businesses.forEach(element => {   
              ...
              CommponentArray.push( <Link to= { { pathname :`/writereview/${object.name}`  , state : object}  } >   < Search_result  key={data.businesses.id} value = {
                    object
                  }  
                  />       </Link>  )
      });
